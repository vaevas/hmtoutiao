<template>
  <div class="fans-container">
    <el-card>
      <div slot="header">
        <go-mianbao>粉丝管理</go-mianbao>
      </div>
      <el-tabs type="border-card" v-model="activeName" @tab-click="setecharts">
        <el-tab-pane label="粉丝列表" name="list">
          <div class="block" v-for="(item,index) in fanslist" :key="index">
            <el-avatar :src="item.photo" :size="80"></el-avatar>
            <p style="font-size:12px">{{item.name}}</p>
            <el-button plain type="primary" size="mini">+关注</el-button>
          </div>
          <el-pagination
            v-if="num>pages.per_page"
            background
...
export default {
  ...
  methods: {
    ...
    async getfans () {
      const {
        data: { data }
      } = await this.$http.get('followers', { params: this.pages })
